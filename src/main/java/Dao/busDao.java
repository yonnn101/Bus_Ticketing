...
public class busDao {
	...
	public boolean addBuses(busData bs){
		...
		try{
			...
			PreparedStatement ps = conn.prepareStatement("insert into busdate(busNo,busName,fromCity,toCity,date,time,ticketPrice)values(?,?,?,?,?,?,?)");
...
public  ArrayList<busData> getAllBuses(){
		...
		try{
			...
			PreparedStatement ps = conn.prepareStatement("select * from busdate order by id ");
...
public boolean updateBus(busData bs){
	...
	boolean f = false;
			...
			try{
				...
				PreparedStatement ps = conn.prepareStatement("update busdate set busNo=?,busName=?,fromCity=?,toCity=?,date=?,time=?,ticketPrice=? where id =?");
...
public busData getBusById(int id){
	...
	try{
		...
		PreparedStatement ps = conn.prepareStatement("select * from busdate where id=?");
...
public boolean deleteBus(int id){
	...
	try{
		...
		PreparedStatement ps = conn.prepareStatement("delete  from busdate where id=?");
		ps.setInt(1, id);
...
public ArrayList<busData> getBusORtCityAndfCity(String fromCity, String toCity, String date){
	...
	try{
		...
		PreparedStatement ps = conn.prepareStatement("select * from busdate where fromCity=? or toCity=? or date=? order by id");
